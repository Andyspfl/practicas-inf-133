...
class RESTRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_DELETE(self):
        if self.path.startswith("/animales/"):
            ...
            if delete_animal: self.response_handler(200, delete_animal)
            else: self.response_handler(404, {"Message":"No se encontro ningun animal con ese id"})
    ...
    def do_GET(self):
        ...
        if parsed_path.path == "/animales":
            if "especie" in query_params:
                ...
                if animales_filtrados:
                    self.response_handler(200, animales_filtrados)
                else: self.response_handler(404, {"Message":"No hay animales de ese genero"})
            ...
            else: self.response_handler(404, {"Mssage":"No hay animales con es ID"})
        else:
